...
| **slice-interestingness (Sarawagi surprise)** | which cell/block of a measure's cube deviates from its rollup expectation — "what moved / where to look" | **SPLIT VERDICT (2026-07-15, Philipp): eval gate YES / engine filter NO.** /ground probe (v1 = unfaithful simplification, recorded; v2 = the published method: surprise at every group-by level of the lattice + variance-aware studentization; probe kept at `scripts/probes/interestingness-sarawagi/` as the harness reference): the statistic is sound with a **measured, law-like power boundary** — a shift δ on a block of n rows is detectable iff `log(1+δ)·√n/cv ≳ ~3.2` (the max-of-field threshold; probe numbers match the formula). At 50 rows/cell: block+20% 16/20 top-5 but margin overlaps clean p95 (3.17 vs 3.62); cell+50% **14/20** — a 3.0σ effect against a 576-cell field whose clean max is ~3.1σ, i.e. **sampling physics, not a method deficiency** (at realistic supports ~1400 rows/cell the same shift is 7σ); block+50% and cell+100% 20/20 **CLEAR**. Variance-aware studentization kills small-support false-fires **8/20 → 0/20**. **Discriminant vs the DAT-545 driver ranking PROVEN**: a whole-region ×1.3 shift leaks into the finest top-5 at exactly the 49% chance baseline — main effects are absorbed by the rollup model; axis stories belong to drivers, surprise owns interactions/localized cells. The pre-registered δ-points sat below the boundary and failed as registered → **never an engine filter**; the deliverable is the **eval family**: localized-slice-shift injection placed by the δ·√n law (above boundary → assert top-k; below → assert honest silence), giving Sarawagi's operators ground truth for the first time. | standardized residual of log cell aggregates vs the **log-linear main-effects (rollup) expectation**, computed per group-by level, **variance-aware** (studentized by √(n̄/n)), MAD scale — **Sarawagi, discovery-driven OLAP exploration (VLDB'98)** | n/a — an eval instrument, not a detector |
